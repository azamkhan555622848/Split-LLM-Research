...
class PrivacyEngine:
    ...
    def add_dp_noise(self, hidden_states: torch.Tensor) -> Tuple[torch.Tensor, float]:
        ...
        Args:
            hidden_states: [seq_len, hidden_dim] tensor (already clipped)
...
def estimate_activation_sensitivity(
    ...
    """
    Empirically estimate L2 sensitivity of activations at the split layer.
    
    Sensitivity = max ||f(x) - f(x')|| over adjacent inputs x, x'.
    For split inference, "adjacent" means inputs differing in one token.
    
    This is used to calibrate DP noise. The theoretical worst-case sensitivity
    may be very large, but empirical sensitivity is typically much smaller.
    
    Args:
        model: The local model shard
        tokenizer: Tokenizer
    ...
    Returns:
        Estimated L2 sensitivity (95th percentile of observed differences)
    """
    sensitivities = []
    ...
    for text in sample_texts[:num_pairs]:
        ...
        # Original activation
        with torch.no_grad():
            h_original = model.forward_to_layer(tokens, split_layer)
        ...
        with torch.no_grad():
            h_perturbed = model.forward_to_layer(perturbed, split_layer)
        
        # L2 distance
        diff = torch.norm(h_original - h_perturbed, p=2).item()
        sensitivities.append(diff)
    
    # Use 95th percentile as sensitivity estimate
    return float(np.percentile(sensitivities, 95))
